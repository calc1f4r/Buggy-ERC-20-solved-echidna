...
import {Challenge17 as Token} from "../src/Challenge17.sol";
...
contract AllInvariants is Token {
    ...
    */
    ...
    function nameAndSymbolShouldBeSame() public view {
        assert(keccak256(abi.encodePacked(name())) == keccak256(abi.encodePacked("BuggyToken17")));
        assert(keccak256(abi.encodePacked(symbol())) == keccak256(abi.encodePacked("BUG17")));
    }
