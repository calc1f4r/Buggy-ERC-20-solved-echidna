...
import {Challenge19 as Token} from "../src/Challenge19.sol";
...
contract AllInvariants is Token {
    ...
    */
    ...
    function nameAndSymbolShouldBeSame() public view {
        assert(keccak256(abi.encodePacked(name())) == keccak256(abi.encodePacked("BuggyToken19")));
        assert(keccak256(abi.encodePacked(symbol())) == keccak256(abi.encodePacked("BUG19")));
    }
