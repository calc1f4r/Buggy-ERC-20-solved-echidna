...
import {Challenge16 as Token} from "../src/Challenge16.sol";
...
contract AllInvariants is Token {
    ...
    */
    ...
    function nameAndSymbolShouldBeSame() public view {
        assert(keccak256(abi.encodePacked(name())) == keccak256(abi.encodePacked("BuggyToken16")));
        assert(keccak256(abi.encodePacked(symbol())) == keccak256(abi.encodePacked("BUG16")));
    }
