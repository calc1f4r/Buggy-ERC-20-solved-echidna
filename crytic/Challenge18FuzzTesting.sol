...
import {Challenge18 as Token} from "../src/Challenge18.sol";
...
contract AllInvariants is Token {
    ...
    */
    ...
    function nameAndSymbolShouldBeSame() public view {
        assert(keccak256(abi.encodePacked(name())) == keccak256(abi.encodePacked("BuggyToken18")));
        assert(keccak256(abi.encodePacked(symbol())) == keccak256(abi.encodePacked("BUG18")));
    }
